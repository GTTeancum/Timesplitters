        ...
        case COP1_S:
            ...
            case COP1_S_DIV:
                return fmt::format("if (ctx->f[{}] == 0.0f) {{ ctx->fcr31 |= 0x100000; /* DZ flag */ "
                                   ...
                                   ft, fd, fs, fd, fs, ft);
            case COP1_S_SQRT:
                // R5900 SQRT.S encodes its unary input in FT, unlike generic MIPS
                // SQRT using FS. Confirmed against the locked PCSX2 FPU.cpp and
                // DisR5900asm.cpp and original TimeSplitters vector-math routines.
