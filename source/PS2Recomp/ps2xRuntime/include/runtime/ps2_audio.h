...
public:
    ...
    static constexpr uint32_t SpuRamBytes = 2u * 1024u * 1024u;
    bool writeSpuMemory(uint32_t byteAddress, const void* source, size_t size);
    bool readSpuMemory(uint32_t byteAddress, void* destination, size_t size) const;
    struct SpuMemoryStats {
        uint64_t writeTransfers = 0, readTransfers = 0;
        uint64_t bytesWritten = 0, bytesRead = 0;
    };
    SpuMemoryStats spuMemoryStats() const;
