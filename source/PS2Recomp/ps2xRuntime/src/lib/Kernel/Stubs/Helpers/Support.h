...
namespace
{
    ...
    // EE variadic integer/pointer/double arguments occupy one 64-bit slot.
    // r4..r11 supply slots 0..7; later slots start at the incoming SP (no
    // O32 home area). An in-memory va_list points into SD-width saved slots.
    // Verified against SLUS_200.90's vafmt 0x002b4868, printf 0x002e5720,
    // sprintf 0x002e5838 and _vfprintf_r 0x002e7650. Do not combine the low
    // words of adjacent GPRs or consume a padding word as another argument.
    class Ps2VarArgCursor
    {
    public:
        Ps2VarArgCursor(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime, int fixedArgs)
            : m_rdram(rdram), m_ctx(ctx), m_runtime(runtime),
              m_stackBase(getRegU32(ctx, 29)),
              m_slotIndex(static_cast<uint32_t>(std::max(fixedArgs, 0)))
        {
        }
        ...
        uint64_t nextU64()
        {
            const uint32_t slot = m_slotIndex++;
            if (slot < 8u)
            {
                uint64_t value = 0u;
                std::memcpy(&value, &m_ctx->r[4u + slot], sizeof(value));
                return value;
            }
            const uint32_t address = m_stackBase + (slot - 8u) * 8u;
            uint32_t low = 0u, high = 0u;
            (void)tryReadWordFromGuest(m_rdram, m_runtime, address, low);
            (void)tryReadWordFromGuest(m_rdram, m_runtime, address + 4u, high);
