...
// Sound RAM byte addressing wraps at 2 MiB; transfer addresses are halfword
// aligned by the SDK's TSA high/low register writes. Larger requests are
// rejected instead of overrunning the host buffer or claiming a partial copy.
bool PS2AudioBackend::writeSpuMemory(uint32_t byteAddress, const void* source, size_t size)
{
    if (size > SpuRamBytes || (size && !source)) return false;
    std::lock_guard<std::mutex> lock(m_mutex);
    const uint32_t start = byteAddress & (SpuRamBytes - 2u);
    const size_t first = std::min(size, static_cast<size_t>(SpuRamBytes - start));
    if (first) std::memcpy(m_impl->spuRam.data()+start, source, first);
    if (size > first) std::memcpy(m_impl->spuRam.data(), static_cast<const uint8_t*>(source)+first, size-first);
    ++m_impl->spuStats.writeTransfers; m_impl->spuStats.bytesWritten += size;
    return true;
}

bool PS2AudioBackend::readSpuMemory(uint32_t byteAddress, void* destination, size_t size) const
{
    if (size > SpuRamBytes || (size && !destination)) return false;
    std::lock_guard<std::mutex> lock(m_mutex);
    const uint32_t start = byteAddress & (SpuRamBytes - 2u);
    const size_t first = std::min(size, static_cast<size_t>(SpuRamBytes - start));
    if (first) std::memcpy(destination, m_impl->spuRam.data()+start, first);
    if (size > first) std::memcpy(static_cast<uint8_t*>(destination)+first, m_impl->spuRam.data(), size-first);
    ++m_impl->spuStats.readTransfers; m_impl->spuStats.bytesRead += size;
    return true;
}

PS2AudioBackend::SpuMemoryStats PS2AudioBackend::spuMemoryStats() const
{
    std::lock_guard<std::mutex> lock(m_mutex);
    return m_impl->spuStats;
}
