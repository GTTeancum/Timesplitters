    ...
    MiniTest::Case("PS2RuntimeExpansion", [](TestCase &tc)
    {
        ...
        tc.Run("sceSdRemote isolates voice transfers from block streaming state", [](TestCase &t)
        {
            ...
            if (!runtime.memory().initialize(PS2_RAM_SIZE) || !runtime.syncCoreSubsystems())
                throw std::runtime_error("audio test runtime bind");
            std::array<uint8_t, 0x800> fixture{}, observed{};
            for (size_t i=0;i<fixture.size();++i) fixture[i]=static_cast<uint8_t>(3u+i*7u);
            if (!runtime.writeIopMemory(0x22000u,fixture.data(),fixture.size()))
                throw std::runtime_error("audio IOP fixture write");
            ...
            t.IsTrue(runtime.audioBackend().readSpuMemory(0x4000u,observed.data(),observed.size()) && fixture==observed,
                     "voice DMA must copy actual IOP bytes to sound RAM");
        ...
        tc.Run("vsprintf reads 64-bit saved EE argument slots", [](TestCase &t)
        {
            PS2Runtime runtime;
            std::vector<uint8_t> ram(PS2_RAM_SIZE, 0u);
            R5900Context ctx{};
            const char fmt[] = "%d %s %d";
            const char word[] = "minute";
            const uint64_t arguments[]{1u, 0x2300u, 7u};
            std::memcpy(ram.data()+0x2100u, fmt, sizeof(fmt));
            std::memcpy(ram.data()+0x2300u, word, sizeof(word));
            std::memcpy(ram.data()+0x2400u, arguments, sizeof(arguments));
            setRegU32(ctx,4,0x2000u);setRegU32(ctx,5,0x2100u);setRegU32(ctx,6,0x2400u);
            ps2_stubs::vsprintf(ram.data(), &ctx, &runtime);
            t.Equals(std::string(reinterpret_cast<char*>(ram.data()+0x2000u)), std::string("1 minute 7"),
                     "padding must not be consumed as the next integer or pointer");
            t.Equals(getRegS32(ctx,2), 10, "return length excludes NUL");
        });

        tc.Run("sprintf consumes one full register for a wide EE argument", [](TestCase &t)
        {
            PS2Runtime runtime;
            std::vector<uint8_t> ram(PS2_RAM_SIZE, 0u);
            R5900Context ctx{};
            const char fmt[] = "%d %lld %.2f %s";
            std::memcpy(ram.data()+0x2100u, fmt, sizeof(fmt));
            std::memcpy(ram.data()+0x2300u, "Tomb", 5);
            setRegU32(ctx,4,0x2000u);setRegU32(ctx,5,0x2100u);setRegU32(ctx,6,9u);
            SET_GPR_U64((&ctx),7,static_cast<uint64_t>(-4294967297LL));
            const double d=1.25;uint64_t bits=0;std::memcpy(&bits,&d,sizeof(bits));
            SET_GPR_U64((&ctx),8,bits);setRegU32(ctx,9,0x2300u);
            ps2_stubs::sprintf(ram.data(), &ctx, &runtime);
            t.Equals(std::string(reinterpret_cast<char*>(ram.data()+0x2000u)), std::string("9 -4294967297 1.25 Tomb"),
                     "wide arguments use neither O32 even-slot alignment nor adjacent GPR low halves");
        });

        tc.Run("sprintf stack spill begins at incoming SP with eight-byte stride", [](TestCase &t)
        {
            PS2Runtime runtime;
            std::vector<uint8_t> ram(PS2_RAM_SIZE, 0u);
            R5900Context ctx{};
            ...
            std::memcpy(ram.data()+0x2100u, fmt, sizeof(fmt));
            setRegU32(ctx,4,0x2000u);setRegU32(ctx,5,0x2100u);setRegU32(ctx,29,0x3000u);
            for(unsigned i=0;i<6u;++i)setRegU32(ctx,6+i,i+1);
            const uint64_t spill[]{7,8,9};std::memcpy(ram.data()+0x3000u,spill,sizeof(spill));
            ps2_stubs::sprintf(ram.data(), &ctx, &runtime);
            t.Equals(std::string(reinterpret_cast<char*>(ram.data()+0x2000u)), std::string("1,2,3,4,5,6,7,8,9"),
                     "first stack argument is at SP, not SP+16");
        });
